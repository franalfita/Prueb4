...
const ClientsEdit = () => {
    ...
    let clientData = JSON.parse(sessionStorage.getItem("clients")); 
    const cancel = () => {
        var {clientName, rol, operatorId} = document.forms[0]; 
        var hasChanges = clientName.value.length > 0 ||  rol.value.length > 0 || operatorId.value.length > 0;
        if(hasChanges){
    ...
    const save = async (event) => {
        ...
        if(errors.length > 0){
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({ "operatorId": config.operatorId, "name": clientName.value,"rol": rol.value,"operatorID": operatorId.value, "active": clientData.active})
              }
                            ...
                            <div className="row">
                                ...
                                <div className="col-4">
                                    <div className="form-group">
                                        ...
                                        <input type="text" name="rol" id="rol" className="form-control" defaultValue={clientData.MSU} required />
                                    </div>
                                </div>
                                <div className="col-4">
                                    <div className="form-group">
                                        <label htmlFor="operadorID" className="control-label">operadorID</label>
                                        <input type="text" name="operadorID" id="operadorID" className="form-control" defaultValue={clientData.price} required />
                                    </div>
                                </div>
                            </div>
